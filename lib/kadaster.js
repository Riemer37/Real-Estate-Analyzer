export async function lookupKadaster(address) {
  ...
  try {
    ...
    // Stap 2: BAG WFS voor gedetailleerde verblijfsobject data (geen API key nodig)
    if (res.bag_id) {
      try {
        ...
        if (wfsR.ok) {
          ...
          if (feat) {
            ...
            res.pand_id       = feat.pandidentificatie ?? null;
          }
        }
      } catch { /* gebruik Locatieserver data als fallback */ }
    }

    // Stap 3: Pand WFS voor bouwjaar en aantal VBO's (splitsing)
    if (res.pand_id || res.bag_id) {
      try {
        const pandId = (res.pand_id ?? res.bag_id).replace(/\D/g, '').padStart(16, '0').slice(0, 16);
        ...
        if (pandR.ok) {
          ...
          const pf = pandData?.features?.[0]?.properties;
          if (pf) {
        ...
        // Haal alle VBO's op in hetzelfde pand inclusief oppervlakte
        ...
        if (vboR.ok) {
          ...
          const features = vboData?.features ?? [];
          res.vbo_count = features.length || null;
          res.is_split  = features.length > 1;
          if (features.length > 1) {
            res.vbo_eenheden = features.map(f => ({
    ...
    // Stap 4: Koopsommen — historische transactieprijzen via PDOK
    if (res.lat && res.lon) {
      try {
        const delta = 0.0003;
        const bbox  = `${res.lon - delta},${res.lat - delta},${res.lon + delta},${res.lat + delta}`;
        const koopUrl = `https://api.pdok.nl/kadaster/koopsommen/ogc/v1/collections/koopsommen/items?bbox=${bbox}&limit=5&sortby=-transactiedatum`;
