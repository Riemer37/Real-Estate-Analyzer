export async function lookupKadaster(address) {
  ...
  try {
    ...
    // Gebruik ruimer bbox (±300m) voor representatief buurtgemiddelde, max 3 jaar oud
    if (res.lat && res.lon) {
      try {
        const delta = 0.003; // ≈ 250–300m in NL — representatieve buurtsteekproef
        ...
        const koopUrl = `https://api.pdok.nl/kadaster/koopsommen/ogc/v1/collections/koopsommen/items?bbox=${bbox}&datetime=${since}/..&limit=20&sortby=-transactiedatum`;
...
// Haal VBO oppervlakte op via BAG WFS spatial query op een punt (RD New of WGS84)
export async function lookupVboOppervlakte(coords) {
  if (!coords || coords.length < 2) return null;
  try {
    const [x, y] = coords;
    // Detecteer coördinatenstelsel: RD New als X > 7000 (Nederland RD grenzen ≈ 7000–300000)
    const isRD = Math.abs(x) > 7000;
    let filter;
    if (isRD) {
      const d = 15;
      filter = `<Filter><BBOX><PropertyName>geometrie</PropertyName><gml:Envelope srsName="urn:ogc:def:crs:EPSG::28992"><gml:lowerCorner>${x - d} ${y - d}</gml:lowerCorner><gml:upperCorner>${x + d} ${y + d}</gml:upperCorner></gml:Envelope></BBOX></Filter>`;
    } else {
      // WGS84 lon/lat — ±0.0001° ≈ 10m
      const d = 0.0001;
      filter = `<Filter><BBOX><PropertyName>geometrie</PropertyName><gml:Envelope srsName="urn:ogc:def:crs:EPSG::4326"><gml:lowerCorner>${y - d} ${x - d}</gml:lowerCorner><gml:upperCorner>${y + d} ${x + d}</gml:upperCorner></gml:Envelope></BBOX></Filter>`;
    }
    const wfsUrl = `https://service.pdok.nl/lv/bag/wfs/v2_0?service=WFS&version=2.0.0&request=GetFeature&typeName=bag:verblijfsobject&outputFormat=application/json&count=1&filter=` + encodeURIComponent(filter);
    const r = await fetch(wfsUrl, { signal: AbortSignal.timeout(8000) });
    if (!r.ok) return null;
    const feat = (await r.json())?.features?.[0]?.properties;
    const opp = feat?.oppervlakte ?? null;
    return opp && opp > 10 ? opp : null;
  } catch { return null; }
}
